...
import type { Task, TaskBarLabels } from '../types/gantt.types';
import { GanttViewMode } from './toolbar/GanttToolbar';
import type { TaskLabel, ProjectMilestone } from '../types/gantt.types';
                ...
                >
                      ...
                      {isGroup ? (
                        <>
                          ...
                          {taskBarLabels.showName && (
                            <span
                              className={cn(
                                'absolute top-0 text-[9px] text-muted-foreground font-medium truncate whitespace-nowrap',
                                textStyleClasses
                              )}
                          ...
                          {(taskBarLabels.showDuration ||
                            taskBarLabels.showDates) && (
                            <span
                              className={cn(
                                'absolute top-1 h-4 flex items-center text-[9px] text-muted-foreground whitespace-nowrap pl-1',
                                textStyleClasses
                              )}
                            ...
                            >
                              ...
                              {taskBarLabels.showName &&
                                position.width > 40 && (
                                  <span
                                    className={cn(
                                      'absolute inset-0 flex items-center px-1.5 text-[9px] text-white font-medium truncate',
                                      textStyleClasses
                                    )}
                                  >
                                    {task.name}
                  ...
                  {task.predecessors?.map((predId) => {
                    ...
                    return (
                      ...
                      >
                        <path
                          d={`M ${predX - Math.min(predX, taskX) + 10} ${
                            predRow < currentRow
                              ? 0
                              : Math.abs(currentRow - predRow) * 28
                          } 
                            L ${predX - Math.min(predX, taskX) + 15} ${
                            predRow < currentRow
                              ? 0
                              : Math.abs(currentRow - predRow) * 28
                          }
                            ...
                            L ${taskX - Math.min(predX, taskX) + 10} ${
                            predRow < currentRow
                              ? Math.abs(currentRow - predRow) * 28
                              : 0
                          }`}
