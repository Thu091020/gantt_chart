...
export default function ProjectDetail() {
  ...
  const initialTab =
    searchParams.get('view') === 'resources' ? 'effort' : 'gantt';
  ...
  const tableScrollRef = useTableScroll<HTMLDivElement>({
  ...
  // Configure Gantt feature on mount (must happen before GanttViewWrapper renders)
  useEffect(() => {
    if (!id) return;
  ...
  const availableEmployees = useMemo(() => {
    ...
    return employees.filter(
      (e) => !memberEmployeeIds.includes(e.id) && e.is_active
    );
  ...
  const isNonWorkingDay = (date: Date) => {
    ...
    if (isSaturday(date))
      return !(settings && isSaturdayWorkingDay(date, settings));
    return false;
  };
  ...
  const projectStats = useMemo(() => {
    if (!project)
      return { totalEffort: 0, avgEffort: 0, workingDays: 0, warningDays: 0 };

    const visibleDays = columns
    ...
    projectMembers.forEach((member) => {
      visibleDays.forEach((day) => {
        ...
        const allocation = allocations.find(
          (a) =>
            a.employee_id === member.employee_id &&
        ...
        const totalDayEffort = allocations
          .filter(
            (a) => a.employee_id === member.employee_id && a.date === dateStr
          )
    ...
    const avgEffort =
      projectMembers.length > 0 ? totalEffort / projectMembers.length : 0;
  ...
  const moveToNextCell = (currentEmpId: string, currentColIdx: number) => {
    ...
    const memberIdx = activeMembersList.findIndex(
      (m) => m.employee_id === currentEmpId
    );
  ...
  const goToPrevious = () => {
    switch (viewMode) {
      ...
      case 'week':
        ...
        break;
      case 'month':
        setStartDate((prev) => addMonths(prev, -6));
        break;
      case 'quarter':
  ...
  return (
    <MainLayout>
      <div className="space-y-2">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <Switch
                          checked={m.is_active}
                          onCheckedChange={() =>
                            handleToggleMemberStatus(m.id, m.is_active)
                          }
                          className="scale-75"
                        />
                        <button
          ...
          <div className="bg-card rounded-lg border border-border p-2 flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm font-bold">
                {activeProjectMembers.length}/{projectMembers.length}
              </p>
            </div>
          </div>
          <div className="bg-card rounded-lg border border-border p-2 flex items-center gap-2">
          ...
          <div className="bg-card rounded-lg border border-border p-2 flex items-center gap-2">
            <div
              className={cn(
                ...
                projectStats.warningDays > 0 ? 'bg-destructive/10' : 'bg-muted'
              )}
            >
              <AlertTriangle
                className={cn(
                  'w-3.5 h-3.5',
                  projectStats.warningDays > 0
                    ? 'text-destructive'
        ...
        <Tabs
          value={tabMode}
          onValueChange={(v) => setTabMode(v as TabMode)}
        ...
        >
          ...
          >
                ...
                {tabMode === 'effort' && project?.last_sync_at && (
                  <div className="text-[10px] text-muted-foreground">
                    ...
                    {project.last_sync_by &&
                      (() => {
                        const syncUser = users.find(
                          (u) => u.id === project.last_sync_by
                        );
                        return syncUser
                          ? ` bởi ${syncUser.full_name || syncUser.email}`
                          : '';
                      })()}
                  </div>
                )}
            ...
            <TabsContent
              value="effort"
              className={cn('mt-2 flex-1', isFullscreen ? 'mb-0' : 'mb-5')}
            >
              {/* Effort Table */}
              <div className="bg-card rounded-lg border border-border overflow-hidden">
                <div className="p-1.5 border-b border-border flex items-center justify-between gap-2 flex-wrap">
                  ...
                  <div className="flex gap-0.5 bg-secondary/50 p-0.5 rounded">
                    {(['day', 'week', 'month', 'quarter'] as ViewMode[]).map(
                      (m) => (
                        <button
                          key={m}
                  ...
                  <DateRangePickerPopup
                    startDate={
                      viewMode === 'custom' ? customStartDate : startDate
                    }
                    endDate={
                      viewMode === 'custom'
                        ? customEndDate
                ...
                {projectMembers.length === 0 ? (
                  ...
                  <div
                    ref={tableScrollRef}
                    className={cn(
                      'overflow-x-auto overflow-y-auto scrollbar-thin',
                      isFullscreen
                    ...
                    <table className="w-full min-w-max">
                      <thead>
                        <tr className="bg-secondary/50">
                          <th className="sticky left-0 bg-secondary z-10 px-2 py-1.5 text-left text-[10px] font-semibold min-w-[140px]">
                            Thành viên
                          </th>
                          <th className="sticky left-[140px] bg-secondary z-10 px-2 py-1.5 text-left text-[10px] font-semibold min-w-[100px]">
                            Vị trí
                          </th>
                          {columns.map((col, idx) => {
                            const isTodayCol =
                              !col.isAggregate &&
                              isSameDay(col.date, new Date());
                            return (
                              <th
                                key={idx}
                                className={cn(
                        ...
                        {projectMembers.map((member, idx) => {
                          ...
                          return (
                            <tr
                              ...
                              title={
                                isInactive
                                  ? 'Thành viên đã inactive, không thể chỉnh sửa effort'
                                  : undefined
                              }
                            >
                              <td
                                className={cn(
                                  'sticky left-0 z-10 px-2 py-1',
                                  rowBg,
                                  isInactive && 'opacity-50'
                                )}
                              >
                              ...
                              <td
                                className={cn(
                                  'sticky left-[140px] z-10 px-2 py-1',
                                  rowBg,
                                  isInactive && 'opacity-50'
                                )}
                              >
                              ...
                              {columns.map((col, colIdx) => {
                                ...
                                const isEditing =
                                  editingCell?.empId === member.employee_id &&
                                  editingCell?.colIdx === colIdx;
                                const isDisabled =
                                  !col.isAggregate && isNonWorkingDay(col.date);
                                const isOver =
                                  !col.isAggregate &&
                                  getTotalEffort(member.employee_id, col.date) >
                                    1;
                                const allocationSource = getAllocationSource(
                                  member.employee_id,
                                  col
                                );
                                const isManualEdit =
                                  ...
                                  <td
                                    ...
                                    className={cn(
                                      ...
                                      !isDisabled &&
                                        ...
                                        !isEditing &&
                                        !isInactive &&
                                        'cursor-cell hover:ring-1 hover:ring-primary/50 hover:ring-inset',
                                      ...
                                      <div className="w-full h-full flex items-center justify-center relative">
                                        ...
                                        {isOver && !isDisabled && (
                                          <AllocationConflictTooltip
                                            ...
                                            date={col.date}
                                            allocations={getEmployeeAllocationsOnDate(
                                              member.employee_id,
                                              col.date
                                            )}
      ...
      {/* Fill Range Dialog */}
      <Dialog
        open={!!fillRangeDialog}
        onOpenChange={(open) => !open && setFillRangeDialog(null)}
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-sm">
              Điền effort - {fillRangeDialog?.empName}
            </DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button
              ...
              onClick={() => setFillRangeDialog(null)}
            >
              Hủy
            </Button>
            <Button size="sm" className="h-7 text-xs" onClick={handleFillRange}>
              Điền effort
            </Button>
          </DialogFooter>
        </DialogContent>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-2">
              <p>
                ...
                <strong>{deleteConfirmMember?.name}</strong> khỏi dự án?
              </p>
              <p className="text-destructive font-medium">
                ⚠️ Tất cả dữ liệu effort của thành viên này trong dự án sẽ bị
                xóa vĩnh viễn và không thể khôi phục.
              </p>
              <p className="text-muted-foreground">
                💡 Gợi ý: Nếu chỉ muốn tạm ngưng, hãy sử dụng chức năng
                "Inactive" thay vì xóa.
              </p>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
